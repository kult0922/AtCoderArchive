...
void dfs(string v, string p) {
    ...
    for (auto nv : g[v]) {
        ...
        if (contains_key(finished, nv)) continue;

        // サイクルを検出
        // if (seen[nv] && !finished[nv]) {
        if (contains_key(seen, nv) && !contains_key(finished, nv)) {
          cycle = true;
          return;
        }
...
int main(){
  ...
  */
  for(auto m : g){
    string start = m.first;
    if (contains_key(seen, start)) continue;
    dfs(start, "");
  }
