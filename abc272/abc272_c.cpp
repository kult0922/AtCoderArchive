...
int main() {
  ...
  sort(gu.begin(), gu.end(), greater<ll>{});
  sort(ki.begin(), ki.end(), greater<ll>{});

  if (gu.size() < 2 && ki.size() < 2){
    print(-1);
    return 0;
  }
  ...
  if(gu.size() > 1) gu_m = gu[0] + gu[1];
  if(ki.size() > 1) ki_m = ki[0] + ki[1];

  ll ans = max(gu_m, ki_m);
