...
const int di_[] = {1,-1,1,-1};
const int dj_[] = {1,-1,-1,1};
...
int main() {
  ...
  ll v1 = 0;
  ll v2 = 0;
  for(ll i = 0; i < 801; i++)for(ll j= 0; j <  801; j++){
    if (i * i + j * j == m){
      v1 = i;
      v2 = j;
      vs.push_back(make_pair(v1, v2));
    }
  }
  // print(v1, v2);
  V<V<ll>> dist(n, V<ll>(n, -1));
  dist[0][0] = 0;
  ...
  while(!que.empty()){
    ...
    que.pop();
    ll cur_i = point.first;
    ll cur_j = point.second;
    for(auto v: vs){
      ll v1 = v.first;
      ll v2 = v.second;
      rep(i, 2){
        rep(j, 4){
          ll nxt_i = cur_i + (ll)di_[j] * (ll)v1;
          ll nxt_j = cur_j + (ll)dj_[j] * (ll)v2;
          if(!isInside(nxt_i, nxt_j, n, n)) continue;
          if(dist[nxt_i][nxt_j] != -1) continue;
          dist[nxt_i][nxt_j] = dist[cur_i][cur_j] + 1;
          que.push(make_pair(nxt_i, nxt_j));
        }
        swap(v1, v2);
      }
    }
