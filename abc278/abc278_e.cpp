...
ll f(ll level, ll x){
  ...
  if (tot[level] / 2 + 1 < x && x < tot[level]){
    return p[level-1] + 1 + f(level - 1, x - tot[level]/2 - 1);
  }
...
bool isOk (ll s, ll l, ll m){
  if (s == l || (s + 1) % m == l) return true;
  return false;
}

ll sumArr(V<ll>& sumArr, V<ll>& arr, int begin, int end){
  if (begin <= end){
    ll inside = sumArr[end + 1] - sumArr[begin];
    return inside;
  }else{
    // print(sumArr[sumArr.size()-1], inside);
    ll inside = sumArr[begin + 1] - sumArr[end];
    return sumArr[sumArr.size()-1] - inside + arr[begin] + arr[end];
  }
}

bool validH(int h){
  if(0 <= h && h <= 23) return true;
  return false;
}

bool validM(int m){
  if(0 <= m && m <= 59) return true;
  return false;
}
...
pair<int, int> nextime(int h, int m){
  m++;
  if (m >= 60){
    m = 0;
    h++;
...
int countKind(map<int, int> mp, map<int, int> allMap){
  int c = allMap.size();
  for(auto m : mp){
    if(allMap[m.first] == m.second) c--;
  }
...
int main() {
  ...
  cin >> H >> W >> n >> h >> w;
  V<V<int>> a(H, V<int>(W));
  V<V<int>> ans(H-h+1, V<int>(W - w + 1));
  rep(i, H)rep(j, W) cin >> a[i][j];


  for(int si = 0; si < H - h + 1; si++){
    V<int> cnt(n+1);
    int now = 0;
