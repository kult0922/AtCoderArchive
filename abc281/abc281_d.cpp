...
int main() {
  ...
  V<ll> a(n);
  rep(i, n) cin >> a[i];
  V<V<V<ll>>> dp(n+1, V<V<ll>>(k+1, V<ll>(d, -1)));
  // dp[0][0][0] = 0;
  // rep(i, d) dp[0][0][i] = 0;
  rep(i, n) dp[i][0][0] = 0;
  ...
  for(int i = 1; i <= n; i++){
    for(int j = 1; j <= k; j++){
      for(int k = 0; k < d; k++){
        if(dp[i-1][j-1][k] != -1){
          ll new_v = dp[i-1][j-1][k] + a[i-1];
          // i番目を使用
          dp[i][j][new_v % d] = max(dp[i][j][new_v % d], new_v);
        }
        // i番目を不使用
      ...
      /*
      // i番目を使用
      ll new_v = dp[i-1][j-1] + a[i-1];
      if (new_v % d == 0 && dp[i][j] != -1) dp[i][j] = max(dp[i][j], new_v);
      // i番目を不使用
