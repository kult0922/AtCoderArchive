...
int main() {
  ...
  V<map<int, int>> yokoDP(yoko.size() + 2);
  V<map<int, int>> tateDP(tate.size() + 2);

  yokoDP[1][a[0]]++;

  for(int i = 1; i <= yoko.size(); i++){
    for(auto m : yokoDP[i]){
      int n1 = m.first + yoko[i-1];
      int n2 = m.first - yoko[i-1];
      yokoDP[i + 1][n1]++;
      yokoDP[i + 1][n2]++;
    }
  }
  ...
  for(int i = 0; i < tate.size(); i++){
    for(auto m : tateDP[i]){
      int n1 = m.first + tate[i];
      int n2 = m.first - tate[i];
      tateDP[i + 1][n1]++;
      tateDP[i + 1][n2]++;
    }
  }
  ...
  */
  ...
  if(contains_key(yokoDP[yoko.size() + 1], x)){
    yokoOK = true;
    //print("YT");
  }

  if(contains_key(tateDP[tate.size()], y)){
    tateOK = true;
    //print("TT");
  }
