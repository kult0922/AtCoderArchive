  ...
  for(int i = 0; i < n-3; i++){
    for(int j = i + 1; j < n-2; j++){
      for(int k = j + 1; k < n-1; k++){
        for(int l = k + 1; l < n; l++){
          V<int> x = {points[i].first, points[j].first, points[k].first, points[l].first};
          V<int> y = {points[i].second, points[j].second, points[k].second, points[l].second};
          // print(x);
          // print(y);
          if(grid[x[0]][y[0]] != '#') continue;
          if(grid[x[1]][y[1]] != '#') continue;
          if(grid[x[2]][y[2]] != '#') continue;
          if(grid[x[3]][y[3]] != '#') continue;
          ...
          if (fi == 0) continue; // 点が重なる
          if (v[0]==fi && v[1]==fi && v[2]==fi && v[3]==fi && v[4]==fi*2 && v[5]==fi*2){
            ans++;
          }
