...
unordered_map<ll, V<ll>> row_index;
unordered_map<ll, V<ll>> col_index;

ll h, w;

ll f(ll current, V<ll>& indexs, bool plus, ll hosu, ll genkai){
  // print("index current");
  // print(indexs[current]);

  if(indexs.size() > 0){
    auto idx = lower_bound(indexs.begin(), indexs.end(), current) - indexs.begin();
    
    // 障害物判定
    if (plus){
      ll conflict_index = indexs[idx];
      if (current + hosu >= conflict_index){
        return conflict_index - 1;
      }
    }else{
      ll conflict_index = indexs[idx - 1];
      if (current - hosu <= conflict_index){
        return conflict_index + 1;
      }
    }
...
pair<ll, ll> next_place(ll r, ll c, char d, ll hosu){
  // print(row_index[r]);
  // print(col_index[c]);

  if(d == 'L') return make_pair(r, f(c, row_index[r], false, hosu, w));
  if(d == 'R') return make_pair(r, f(c, row_index[r], true, hosu, w));
  if(d == 'D') return make_pair(f(r, col_index[c], true, hosu, h), c);
  if(d == 'U') return make_pair(f(r, col_index[c], false, hosu, h), c);

  return make_pair(f(c, row_index[r], false, hosu, h), c);
...
int main() {
  ...
  rep(i, n){
    ll r = rs[i];
    ll c = cs[i];
    row_index[r].push_back(c);
    col_index[c].push_back(r);

    row_index[r].push_back(0);
    row_index[r].push_back(w+1);
    col_index[c].push_back(0);
    col_index[c].push_back(h+1);
  }
  /*
  rep(i, n){
    ll r = rs[i];
    ll c = cs[i];
    sort(row_index[r].begin(), row_index[r].end());
    sort(col_index[c].begin(), col_index[c].end());
  }
  */
  ...
  rep(i, q){
    ...
    pair<ll, ll> next_p = next_place(cr, cc, d, hosu);
