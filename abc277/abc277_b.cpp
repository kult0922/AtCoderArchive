...
int main() {
  ...
  map<string, int> m3;
  V<char> v1 = {'H' , 'D' , 'C' , 'S' };
  V<char> v2 = {'A' , '2' , '3' , '4' , '5' , '6' , '7' , '8' , '9' , 'T' , 'J' , 'Q' , 'K'};

  rep(i , v1.size()) m1[v1[i]]++;
  rep(i , v2.size()) m2[v2[i]]++;

  rep(i, n){
    if(!contains_key(m1, s[i][0])) ok = false;
  }
  rep(i, n){
    if(!contains_key(m2, s[i][1])) ok = false;
  }

  rep(i, n){
    m3[s[i]]++;
    if(m3[s[i]] > 1) ok = false;
  }
