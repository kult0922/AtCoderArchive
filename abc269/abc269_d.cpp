...
const int hi[] = {-1,0,1,0,-1,1};
const int hj[] = {0,-1,0,1,-1,1};
...
int main() {
  ...
  map<pair<ll, ll>, int> isBlack;
  ll ans = 0;

  rep(i, n) {
    cin >> x[i] >> y[i];
    isBlack[make_pair(x[i], y[i])]++;
  }

  rep(i, n){
    // 未発見
    // print("start", x[i], y[i]);
    if(!contains_key(seen, make_pair(x[i], y[i]))) {
      ans++;
    }
    seen[make_pair(x[i], y[i])]++;

    V<pair<ll, ll>> stack;
    stack.push_back(make_pair(x[i], y[i]));
    while(!stack.empty()){
      pair<ll, ll> c = stack.back();
      stack.pop_back();
      rep(i, 6){
        ll nx = c.first + hj[i];
        ll ny = c.second + hi[i];
        // 黒じゃないとだめ
        if(!contains_key(isBlack, make_pair(nx, ny))) continue;
        // すでにおとずれている
        //print("black", nx, ny);
        if(contains_key(seen, make_pair(nx, ny))) continue;
        //print("seen", nx, ny);
        seen[make_pair(nx, ny)]++;
