...
void dfs(int v, int color, int base){
  //print(v, color);
  if (colors[v] != 0) return;
  colors[v] = color;

  for(int nv : g[v]){
    if (colors[nv] != 0) continue;
    if (color == base) dfs(nv, base + 1, base);
    if (color != base) dfs(nv, base, base);
  }
}
...
int main() {
  ...
  rep(i, N){
    ...
    int tot = N - C[current_color];

    int neighbor = 0;
    for(int nv : g[i]){
      if (colors[nv] != current_color) neighbor++;
    }
