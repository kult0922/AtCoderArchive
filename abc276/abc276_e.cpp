...
int main() {
  ...
  rep(i, h)rep(j, w){
    if(grid[i][j] == 'S') {
      sr = i;
      sc = j;
      flat[t(i, j, h, w)] = grid[i][j];
    }
  }

  V<pair<int, int>> neighbor;
  rep(i, 4){
    int nr = sr + di[i];
    int nc = sc + dj[i];
    if (isInside(nr, nc, h, w) && grid[nr][nc] != '#'){
      neighbor.push_back(make_pair(sr + di[i], sc + dj[i]));
    }
  }
  ...
  for(int k = 0; k < h; k++){
    for(int l = 0; l < w; l++){
      rep(m, 4){
        int nr = k + di[m];
        int nc = l + dj[m];
        if (isInside(nr, nc, h, w) && grid[nr][nc] != '#' && grid[k][l] != '#'){
          g[t(k, l, h, w)].push_back(t(nr, nc, h, w));
        }
      }
    }
  }
  ...
  for(int k = 0; k < h; k++){
    for(int l = 0; l < w; l++){
      for(int nt : g[t(k, l, h, w)]){
        if(grid[k][l] == 'S' || flat[nt] == 'S') continue;
        uf.unite(t(k, l, h, w), nt);
      }
    }
  }
  ...
  for(int i = 0; i < neighbor.size(); i++){
    for(int j = i + 1; j < neighbor.size(); j++){
      ...
      if(uf.isSame(t(p1.first, p1.second, h, w), t(p2.first, p2.second, h, w))){
        ok = true;
        break;
      }
    }
